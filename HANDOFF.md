...
`exp(-0.5 (d / sigma)^2)` with `sigma = control_radius / 2`, contributions
are fixed-point quantized (`1/2^20`) so summation is exactly order-invariant,
and a cell's control share is `I_t / (I_0 + I_1 + 0.125)` — margin of force
matters, unreached ground stays neutral, a lone soldier on a cell holds
~89%. Scoring, reward, and the winner integral use the continuous shares;
display and local observations use majority-rule ternary ownership
(share > 1/2). Control shares are zero at frame zero (first computed after
step one), so `learner_territory` starts at 0 each episode. SIMULATOR_RULES
is at version 0.5.

## Presence control and integral scoring (v16, rules 0.4, superseded)
